!function ($) {
    var pluginName = "yiiDynamicForm", regexID = /^(.+?)([-\d-]{1,})(.+)$/i,
        regexName = /(^.+?)([\[\d{1,}\]]{1,})(\[.+\]$)/i;
    ...
    var events = {
        ...
        $template.find("div[data-dynamicform]").each(function () {
            var widgetOptions = eval($(this).attr("data-dynamicform")), item;
            1 < $(widgetOptions.widgetItem).length && (item = $(this).find(widgetOptions.widgetItem).first()[0].outerHTML, $(this).find(widgetOptions.widgetBody).html(item))
        }), $template.find("input, textarea, select").each(function () {
            var e, t, i, n;
            $(this).is(":checkbox") || $(this).is(":radio") ? (e = $(this).is(":checkbox") ? "checkbox" : "radio", t = $(this).attr("name"), i = $template.find('input[type="hidden"][name="' + t + '"]').first(), n = $template.find('input[type="' + e + '"][name="' + t + '"]').length, i && 1 === n && ($(this).val(1), i.val(0)), $(this).prop("checked", !1)) : $(this).is("select") ? $(this).find("option:selected").removeAttr("selected") : $(this).val("")
        });
        var yiiActiveFormData = $("#" + widgetOptions.formId).yiiActiveForm("data");
        return $template.find("." + yiiActiveFormData.settings.errorCssClass).removeClass(yiiActiveFormData.settings.errorCssClass), $template.find("." + yiiActiveFormData.settings.successCssClass).removeClass(yiiActiveFormData.settings.successCssClass), $template
        ...
        }), level = _getLevel($elem.closest("." + widgetOptions.widgetContainer)), widgetOptionsRoot = _getWidgetOptionsRoot(widgetOptions), identifiers = _createIdentifiers(level), identifiers[0] = $(widgetOptionsRoot.widgetItem).length - 1, identifiers[level] = count - 1, widgetOptions.fields.forEach(function (e) {
        ...
        var n = _count(i, e);
        n > e.min && ($todelete = i.closest(e.widgetItem), !1 !== $("." + e.widgetContainer).triggerHandler(events.beforeDelete, $todelete) && (_removeValidations($todelete, e, n), $todelete.remove(), _updateAttributes(e), _restoreSpecialJs(e), _fixFormValidaton(e), $("." + e.widgetContainer).triggerHandler(events.afterDelete)))
    }, _updateAttrID = function ($elem, index) {
        var widgetOptions = eval($elem.closest("div[data-dynamicform]").attr("data-dynamicform")),
            id = $elem.attr("id"), newID = id;
        if (void 0 !== id) {
            var matches = id.match(regexID);
            if (matches && 4 === matches.length) {
                matches[2] = matches[2].substring(1, matches[2].length - 1);
                var identifiers = matches[2].split("-");
                if (identifiers[0] = index, 1 < identifiers.length) {
                    var widgetsOptions = [];
                    $elem.parents("div[data-dynamicform]").each(function (i) {
                        widgetsOptions[i] = eval($(this).attr("data-dynamicform"))
                    }), widgetsOptions = widgetsOptions.reverse();
                    for (var i = identifiers.length - 1; 1 <= i; i--) identifiers[i] = $elem.closest(widgetsOptions[i].widgetItem).index()
                }
        ...
        void 0 !== t && ((t = $.extend(!0, {}, t)).id = i, t.container = ".field-" + i, t.input = "#" + i, t.name = n, t.value = $("#" + i).val(), t.status = 0, "undefined" !== $("#" + e.formId).yiiActiveForm("find", i) && $("#" + e.formId).yiiActiveForm("remove", i), $("#" + e.formId).yiiActiveForm("add", t))
        ...
        0 < $hasInputmask.length && $hasInputmask.each(function () {
        ...
        0 < $hasDatepicker.length && $hasDatepicker.each(function () {
        ...
        0 < $hasTimepicker.length && $hasTimepicker.each(function () {
        ...
        0 < $hasMaskmoney.length && $hasMaskmoney.each(function () {
            $(this).parent().find("input").removeData().off();
            var id = "#" + $(this).attr("id"), displayID = id + "-disp";
        ...
        0 < $hasFileinput.length && $hasFileinput.each(function () {
        ...
        0 < $hasTouchSpin.length && $hasTouchSpin.each(function () {
        ...
        0 < $hasSpectrum.length && $hasSpectrum.each(function () {
            var id = "#" + $(this).attr("id"), sourceID = id + "-source";
        ...
        0 < $hasDepdrop.length && $hasDepdrop.each(function () {
        ...
        0 < $hasSelect2.length && $hasSelect2.each(function () {
            var id = $(this).attr("id"), configSelect2 = eval($(this).attr("data-krajee-select2"));
            ...
            configDepdrop && (configDepdrop = $.extend(!0, {}, configDepdrop), $(this).removeData().off(), $(this).unbind(), _restoreKrajeeDepdrop($(this))),
                $.when($("#" + id).select2(configSelect2)).done(s2LoadingFunc(id, ".select2-container--krajee"));
            var kvClose = "kv_close_" + id.replace(/\-/g, "_"), loadingText;
            $("#" + id).on("select2:opening", function (e) {
                s2OpenFunc(id, kvClose, e)
            }), $("#" + id).on("select2:unselect", function () {
                window[kvClose] = !0
            }), configDepdrop && (loadingText = configDepdrop.loadingText ? configDepdrop.loadingText : "Loading ...", initDepdropS2(id, loadingText))
        })
    }
}(window.jQuery);